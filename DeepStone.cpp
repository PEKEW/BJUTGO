...
static void parse_commandline(int argc, char *argv[]) {
    ...
    gen_desc.add_options()
        ...
        ("benchmark", "Test network and exit. Default args:\n-v3200 --noponder "
                      "-m0 -t1 -s1.")
        ("rollout,o", "Use rollout or not.")
        ("rollout_mode,e", "Use this command,rollout will return 1or-1.")
        ;
#ifdef USE_OPENCL
    ...
    if (vm.count("rollout")) {
        cfg_use_rollout = true;  // Set this early to avoid unnecessary output.
    }
    if (vm.count("rollout_mode")) {
        cfg_rollout_mode = true;  // Set this early to avoid unnecessary output.
    }
