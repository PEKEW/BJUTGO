...
inline bool FullBoard::IsSelfAtariNakade(int v) const{ 
	...
	int64 space_hash[2] = {zobrist1.hash[0][0][EBVCNT/2], zobrist1.hash[0][0][EBVCNT/2]};
			...
			if(ren[ren_idx[v_nbr]].size < 5){
					...
					== checked_idx[pl].end())
				{
					...
					do{
						...
						space_hash[pl] ^= zobrist1.hash[0][0][v_tmp - v + EBVCNT/2];
...
bool FullBoard::IsEyeShape(int pl, int v) const{
...
bool FullBoard::IsFalseEye(int v) const{
	assert(color[v] == 0);
...
bool FullBoard::IsLegal(int pl, int v) const{
    assert(v <= PASS_AQ);

	if (v == PASS_AQ) return true;
	if (color[v] != 0 || v == ko) return false;

	return ptn[v].IsLegal(pl);
}

bool FullBoard::Semeai2(std::vector<int>& patr_rens, std::vector<int>& her_libs){
	her_libs.clear();
...
bool FullBoard::Semeai3(std::vector<int>& lib3_rens, std::vector<int>& her_libs){
...
int FullBoard::SelectMove(){

	int next_move = PASS_AQ;
...
int FullBoard::SelectRandomMove(){
...
inline void FullBoard::SubProbDist(){
    if (prev_move[her] < PASS_AQ) {
...
inline void FullBoard::SubPrevPtn(){
    if(prev_ptn_prob == 0.0 || prev_move[her] >= PASS_AQ) return;
...
inline void FullBoard::AddProb(int pl, int v, double add_prob){
	if(v >= PASS_AQ || prob[pl][v] == 0) return;

	sum_prob_rank[pl][etoy[v] - 1] += (add_prob - 1) * prob[pl][v];
	prob[pl][v] *= add_prob;
}

inline void FullBoard::UpdatePrevPtn(int v){
...
inline void FullBoard::PlaceStone(int v){
    assert(color[v] == 0);
...
inline void FullBoard::MergeRen(int v_base, int v_add){
    // 1. Ren�N���X�̌���
	//    Merge of Ren class.
...
inline void FullBoard::RemoveStone(int v){
    assert(color[v] > 1);
...
inline void FullBoard::AddUpdatedPtn(int v){
    if (!is_ptn_updated[v]) {
		//�m���X�V�t���O�𗧂Ă�
		is_ptn_updated[v] = true;
...
inline void FullBoard::RemoveRen(int v){
    // 1. ���ׂĂ̐΂����
	...
	if(spaces.size()>=3 && spaces.size()<=6){
		...
		for(auto v_space: spaces){
			...
			//    Calculate Zobrist hash relative to the center position.
			space_hash ^= zobrist1.hash[0][0][v_space - v + EBVCNT / 2];
		}
...
inline void FullBoard::CancelAtari(int v){
    assert(color[v] > 1);
...
inline void FullBoard::CancelPreAtari(int v){
    assert(color[v] > 1);
...
inline void FullBoard::SetPreAtari(int v){
    assert(color[v] > 1);
...
inline bool FullBoard::IsSelfAtari(int pl, int v) const{
...
inline void FullBoard::UpdateProbAll(){
...
inline void FullBoard::AddProbDist(int v){
...
inline void FullBoard::SetAtari(int v){
    assert(color[v] > 1);
...
void FullBoard::PlayLegal(int v){
	if(v==PASS) 
		v=PASS_AQ;
	assert(v <= PASS_AQ);
	assert(v == PASS_AQ || color[v] == 0);
	assert(v != ko);
...
void FullBoard::ReplaceProb(int pl, int v, double new_prob){
    sum_prob_rank[pl][etoy[v] - 1] += new_prob - prob[pl][v];
	prob[pl][v] = new_prob;
}

void FullBoard::AddProbPtn12() {
