...
void Training::record(GameState& state, UCTNode& root) {
  ...
  //if(cfg_use_rollout){
        step.root_uct_winrate = root.get_nn_eval(step.to_move)*lambda+root.get_rollouts(step.to_move)*(1-lambda);
        step.child_uct_winrate = best_node.get_nn_eval(step.to_move)*lambda+ best_node.get_rollouts(step.to_move)*(1-lambda);
    //}
    //else{
        //step.root_uct_winrate = root.get_nn_eval(step.to_move);
        //step.child_uct_winrate = best_node.get_nn_eval(step.to_move);
    //}
