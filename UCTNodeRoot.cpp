...
void UCTNode::prepare_root_node(int color,
    ...
    float root_eval;
    float root_rollout = 0.0;
    const auto had_children = has_children();
    if (expandable()) {
        create_children(nodes, root_state, root_eval);
    }
    if (had_children) {
        root_eval = get_nn_eval(color);
        if(cfg_use_rollout)
            root_rollout = get_rollouts(color);
    } else {
        update(root_eval, root_rollout);
        root_eval = (color == FastBoard::BLACK ? root_eval : 1.0f - root_eval);
        if(cfg_use_rollout)
            root_rollout = (color == FastBoard::BLACK ? root_rollout : 1.0f - root_rollout);
    }
