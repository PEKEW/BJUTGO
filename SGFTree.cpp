...
std::string SGFTree::state_to_string(GameState& pstate, int compcolor) {
    ...
    *state = pstate;
    ...
    auto DeepStone_name = std::string{PROGRAM_NAME};
    DeepStone_name.append(" " + std::string(PROGRAM_VERSION));
    if (!cfg_weightsfile.empty()) {
        ...
        DeepStone_name.append(" " + cfg_weightsfile.substr(pos, 8));
    }

    if (compcolor == FastBoard::WHITE) {
        header.append("PW[" + DeepStone_name + "]");
        header.append("PB[Human]");
    } else {
        header.append("PB[" + DeepStone_name + "]");
